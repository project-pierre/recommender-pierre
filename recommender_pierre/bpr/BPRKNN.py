...
#         learning rate)
#
#     regularization : int, default 0.01
#         regularizationularization term for the user and item latent factors
#
...
#     Attributes
#     ----------
#     user_factors : 2d ndarray, shape [n_users, factors]
#         User latent factors learnt
#
#     item_factors : 2d ndarray, shape [n_items, factors]
...
#         self.user_factors = rstate.normal(size=(n_users, self.factors))
#         self.item_factors = rstate.normal(size=(n_items, self.factors))
#
#         # progress bar for training iteration if verbose is turned on
#         loop = range(self.iterations)
#         if self.verbose:
#             loop = trange(self.iterations, desc=self.__class__.__name__)
#
#         for _ in loop:
...
#
#         # repeat the 1 dimension sigmoid factors times so
#         # the dimension will match when doing the update
#         sigmoid_tiled = np.tile(sigmoid, (self.factors, 1)).T
#
#         # update using gradient descent
#         grad_u = sigmoid_tiled * (item_j - item_i) + self.regularization * user_u
#         grad_i = sigmoid_tiled * -user_u + self.regularization * item_i
#         grad_j = sigmoid_tiled * user_u + self.regularization * item_j
...
#             top_n, _ = self.recommend_user(ratings, user, N)
...
#         best_scores = np.take(scores, top_n)
#
#         return top_n, best_scores
#
...
#
#     def recommender_for_one(self, X_train, user, k=10):
#         rec, best_scores = self.recommend_user(ratings=X_train, user=user, N=k)
#         return pd.DataFrame(
#             {
#                 self.user_label: [user] * len(rec),
#                 self.item_label: rec,
#                 self.transaction_label: best_scores,
#             }
#         ).sort_values(self.transaction_label, ascending=False)
#
#     def recommend_for_all(self, X_train, users, k=10):
#         user_preds = [
#             self.recommender_for_one(X_train, user, k=k)
#             for user in users
#         ]
...
#             X_train=X_train, users=users_ids, k=self.list_size
#         )
